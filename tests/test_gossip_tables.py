...
async def test_user_advertise_updates_and_gossips():
    ...
    async with websockets.connect(f"ws://{host}:{port}") as ws_a, \
        ...
        join = lambda sid: json.dumps({
            "type":"SERVER_HELLO_JOIN","from":sid,"to":f"{host}:{port}",
            "ts":ms(),"payload":{"host":host,"port":7777,"pubkey":"AA"},"sig":"AA"
        }, separators=(",", ":"), sort_keys=True)

        await ws_a.send(join(server_a)); _ = await ws_a.recv()
        await ws_b.send(join(server_b)); _ = await ws_b.recv()

        # A sends USER_ADVERTISE for user_u, should update table and gossip to B
        adv = json.dumps({
            "type":"USER_ADVERTISE","from":server_a,"to":"*","ts":ms(),
            "payload":{"user_id":user_u,"server_id":server_a,"meta":{}}, "sig":"AA"
        }, separators=(",", ":"), sort_keys=True)
        await ws_a.send(adv)
